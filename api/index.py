...
AIROUTER_API_KEY = os.environ.get("AIROUTER_API_KEY")

supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

# Initializing OpenAI Client for AIRouter
client = OpenAI(
    base_url="https://api.airouter.in/v1",  # Fixed missing closing quote
    api_key=AIROUTER_API_KEY,  # Matched variable name
)

# Make sure this model ID matches an available model in your AIRouter catalog
...
def clean_context_retrieval(retrieved_chunks):
    ...
    with quiz questions, options, or blank fills rather than narrative/lesson
    content.
    """
    valid_chunks = []
    ...
    for chunk in retrieved_chunks:
        ...
        option_markers = sum(
            [
                content.count(f"({char})")
                for char in ["i", "ii", "iii", "iv", "a", "b", "c", "d"]
            ]
        )
        mcq_choices = sum(
            [content.count(f"{char}. ") for char in ["A", "B", "C", "D"]]
        )
...
ADVICE: (one line on what to fix)"""
    ...
    # 3-Attempt Retry Loop to handle AIRouter Rate Limits
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            print(f"⚠️ AIRouter Error on attempt {attempt + 1}: {str(e)}")
